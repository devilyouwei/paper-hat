...
The abstraction and replay-builder modules reuse the Cortex itself (with dedicated prompts), making HAT lightweight and self-contained.  The selection scoring uses the Cortex's own predictive entropy for $\uncertainty$, cosine similarity in the model's embedding space for $\novelty$, and binary user-correction flags for $\feedback$.

\paragraph{Hyperparameters.}
We set $\alpha = 0.4$, $\beta = 0.4$, $\gamma = 0.2$ in Eq.~\eqref{eq:selection_score} based on validation-set tuning.  The write threshold is $\tau = 0.3$.  Oracle consultation is triggered when $\uncertainty > \tau_\oracle = 0.7$ or when $\feedback = 1$.  SWS training uses LoRA (rank 16) with AdamW ($\eta = 2 \times 10^{-5}$), batch size 32, and 3 epochs per cycle.  The EWC coefficient is $\mu = 0.1$.  A SWS cycle is triggered every 500 interactions.
