...
import type { EmbeddingCatalogItem } from "@/api/types";
import { useEmbeddingModelsStore } from "@/stores/embeddingModels";
import { formatBytes } from "@/utils/format";

const props = defineProps<{ item: EmbeddingCatalogItem }>();

const store = useEmbeddingModelsStore();
const message = useMessage();
const dialog = useDialog();

const dl = computed(() => store.downloadFor(props.item.backend, props.item.id));
...
async function activate() {
  try {
    await store.activate(props.item.backend, props.item.id);
...
function download() {
  store.startDownload(props.item.backend, props.item.id);
}

async function cancel() {
  await store.cancelDownload(props.item.backend, props.item.id);
...
function remove() {
  dialog.warning({
    ...
    onPositiveClick: async () => {
      try {
        await store.remove(props.item.backend, props.item.id);
...
<template>
  ...
  >
    ...
    <div class="meta">
      ...
      <span v-if="item.size_gb" class="hat-muted">
        {{ item.size_gb.toFixed(1) }} GB
      </span>
    </div>
