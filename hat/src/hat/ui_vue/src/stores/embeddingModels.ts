...
import * as api from "@/api/embeddingModels";
import type { EmbeddingCatalogItem, ActiveEmbedder } from "@/api/types";
...
export const useEmbeddingModelsStore = defineStore("embeddingModels", () => {
  const backend = ref<string>("mlx_embed");
  const items = ref<EmbeddingCatalogItem[]>([]);
  const active = ref<ActiveEmbedder | null>(null);
  const downloads = ref<Map<string, EmbedDownloadState>>(new Map());
  const loading = ref(false);

  const dlKey = (b: string, id: string) => `${b}/${id}`;
  ...
  function startDownload(b: string, id: string) {
    ...
    const state: EmbedDownloadState = {
      backend: b,
      id,
      bytes_done: 0,
      bytes_total: 0,
    ...
    const update = (patch: Partial<EmbedDownloadState>) => {
  ...
  function downloadFor(b: string, id: string): EmbedDownloadState | undefined {
